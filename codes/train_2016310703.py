...
def get_args():
    ...
    parser.add_argument("--train_set", type=str, default="train",
                        help="For both VOC2007 and 2012, you could choose 3 different datasets: train, trainval and val.")
    parser.add_argument("--test_set", type=str, default="val",
                        help="For both VOC2007 and 2012, you could choose 3 different datasets: train, trainval and val.")
    parser.add_argument("--year", type=str, default="2012", help="The year of dataset (2012)")
    parser.add_argument("--data_path", type=str, default="data/train_val", help="the root folder of dataset")
    parser.add_argument("--val_ratio", type=float, default = 0.2, help="the ratio of validation data")
...
def train(opt):
    ...
    test_params = {"batch_size": opt.batch_size,
                   ...
                   "drop_last": False,
                   "collate_fn": custom_collate_fn}
    make_train_val_indices(opt.data_path, opt.val_ratio)
    ...
    criterion = YoloLoss(num_classes = training_set.num_classes, 
                         anchors = model.anchors, 
                         coord_scale = opt.coord_scale,  
                         noobject_scale = opt.noobject_scale,
                         object_scale = opt.object_scale,
                         class_scale = opt.class_scale)
    ...
    for epoch in range(opt.num_epoches):
        ...
        if epoch % opt.test_interval == 0:
            ...
            if te_loss + opt.es_min_delta < best_loss:
                best_loss = te_loss
                best_epoch = epoch
                torch.save(model, opt.saved_path + os.sep + "model_2016310703.pt")
